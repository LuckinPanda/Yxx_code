"""
Fine-tune Stage-R-pre V8: 基于原始最佳权重 + 感知损失微调

保持原始架构，使用预训练的VGG特征进行感知损失微调
"""
import argparse
import logging
import random
...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self, device):
        super().__init__()
        vgg = models.vgg16(pretrained=True).features[:16].eval()  # conv3_3
    ...
    def forward(self, pred, target):
        pred_feat = self.vgg(self.normalize(pred.clamp(0, 1)))
        target_feat = self.vgg(self.normalize(target.clamp(0, 1)))
        return F.l1_loss(pred_feat, target_feat)


def main() -> None:
    logger = setup_logger("stage_R_pre_v8")

    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="configs/stage_R_pre_v8.yaml")
    ...
    # 加载预训练的去噪权重
    pretrain_ckpt = cfg["ckpt"].get("denoise_ckpt_path", "checkpoints/denoise_pre_ckpt.pth.bak_mar7")
    logger.info(f"Loading pre-trained weights from: {pretrain_ckpt}")
    model.adarenet.load_state_dict(torch.load(pretrain_ckpt, map_location=device, weights_only=True))

    for p in model.illumination.parameters():
        p.requires_grad = False
    model.illumination.eval()
    logger.info("IlluminationNet frozen, AdaReNet trainable (fine-tune)")
    ...
    periodic_dir = save_dir / "stage_R_pre_v8"
    ...
    lambda_perceptual = cfg["train"].get("lambda_perceptual", 0.05)
    ...
    logger.info("=" * 80)
    logger.info("Stage-R-pre V8: 感知损失微调")
    logger.info(f"Epochs: {epochs}, mask_prob: {mask_prob}")
    logger.info(f"Weights: perceptual={lambda_perceptual}, grad={lambda_grad}, color={lambda_color}, delta={lambda_delta}")
    logger.info("=" * 80)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch in enumerate(tqdm(cached_loader, desc=f"V8 Epoch {epoch}", ncols=80)):
            ...
            with torch.amp.autocast(device_type=device.type, enabled=use_amp):
                ...
                # 2) Perceptual loss
                loss_perc = perceptual_loss_fn(r_tilde, p_ref)
                ...
                loss = loss_ss + lambda_perceptual * loss_perc + lambda_grad * loss_grad + lambda_color * loss_color + loss_reg
            ...
            if (step + 1) % log_interval == 0:
                ...
                msg = (
                    f"[V8][E{epoch}][S{step+1}] loss={loss.item():.4f} "
                    f"ss={loss_ss.item():.4f} perc={loss_perc.item():.4f} "
        ...
        msg = (
            f"[V8][E{epoch}] avg_loss={avg_loss:.4f} "
            f"ss={epoch_ss/n:.4f} perc={epoch_perc/n:.4f} grad={epoch_grad/n:.4f}"
        )
        print(msg)
        logger.info(msg)
